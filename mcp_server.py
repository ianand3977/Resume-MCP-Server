...
TOKEN = "fcac7b5efcbb"
MY_NUMBER = "919421903054"  # Insert your number {91}{Your number}
...
async def resume() -> str:
    ...
    Reads 'resume.txt' from the current directory and returns its contents as markdown.
    Handles errors gracefully.
    """
    try:
        resume_path = Path("resume.txt")
        if not resume_path.exists():
            return "<error>Resume file 'resume.txt' not found.</error>"
        content = resume_path.read_text(encoding="utf-8")
        # If the resume is already in markdown or plain text, just return it
        return content
